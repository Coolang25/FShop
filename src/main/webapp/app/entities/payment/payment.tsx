  ...
  return (
    <div>
      <h2 id="payment-heading" data-cy="PaymentHeading">
        <Translate contentKey="fShopApp.payment.home.title">Payments</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="fShopApp.payment.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/payment/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="fShopApp.payment.home.createLabel">Create new Payment</Translate>
          </Link>
        </div>
      </h2>
        ...
        {paymentList && paymentList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="fShopApp.payment.id">ID</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
                ...
                <th>
                  <Translate contentKey="fShopApp.payment.order">Order</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {paymentList.map((payment, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>
                    <Translate contentKey={`fShopApp.PaymentMethod.${payment.method}`} />
                  </td>
                  <td>
                    <Translate contentKey={`fShopApp.PaymentStatus.${payment.status}`} />
                  </td>
                  <td>{payment.amount}</td>
                  <td>{payment.transactionId}</td>
                  <td>{payment.createdAt ? <TextFormat type="date" value={payment.createdAt} format={APP_DATE_FORMAT} /> : null}</td>
                  <td>{payment.order ? <Link to={`/shop-order/${payment.order.id}`}>{payment.order.id}</Link> : ''}</td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="fShopApp.payment.home.notFound">No Payments found</Translate>
            </div>
          )
        )}
      </div>
